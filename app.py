...
# --- الدالة المحدثة لحساب ميزان الشيخ والمسجد بدقة متناهية ---
def calculate_sheikh_final_balance(df):
  if df.empty:
    return 0.0, 0.0, 0.0

  paid_out = 0.0  # مبالغ دفعها الشيخ للمسجد من جيبه
  received_back = 0.0  # مبالغ سحبها/استلمها الشيخ من الصناديق

  for _, row in df.iterrows():
    is_sheikh_fund = row["fund"] == "ذمة وسلف الشيخ عبد الكريم"
    is_sheikh_acc = row["account_type"] == "حساب الشيخ عبد الكريم"

    # 1. إيداع أموال من جيب الشيخ إلى المسجد/الصناديق
    if row["type"] == "قبض" and (is_sheikh_acc or is_sheikh_fund):
      paid_out += row["total_usd"]

    # 2. صرف الشيخ من صندوق ذمته لصالح مصاريف المسجد
    elif row["type"] == "صرف" and is_sheikh_fund:
      paid_out += row["total_usd"]

    # 3. أخذ/سحب الشيخ أموالاً من الصناديق الأخرى (كالزكاة أو العامة) لحسابه الشخصي/ذمته
    elif row["type"] == "صرف" and is_sheikh_acc and not is_sheikh_fund:
      received_back += row["total_usd"]

  # الميزان الصافي: (الموجب = مستحق للشيخ | السالب = مطلب على الشيخ)
...
if page == "🏠 الرئيسية (لوحة التحكم)":
  ...
  selected_month = col_m.selectbox(
      ...
      key="sal_m_select_v44",
  )
  selected_year = col_y.number_input(
      "السنة:", min_value=2020, value=datetime.now().year, key="sal_y_select_v44"
  )
  ...
  for f in df_funds["name"]:
    if f == "ذمة وسلف الشيخ عبد الكريم":
      if net_sheikh_status > 0:
        ...
        status_text = (
            f"${abs(net_sheikh_status):,.0f} (مطلب عليك للمسجد / الصناديق)"
        )
      else:
...
# --- 2. القيود اليومية ---
  ...
  t_date = col1.date_input("التاريخ", datetime.now(), key="q_date_v44")
  t_type = col2.selectbox("نوع العملية", ["قبض", "صرف"], key="q_type_v44")
  ...
  fund = col1.selectbox("الصندوق المتأثر", funds_list, key="q_fund_v44")
  ...
  if account_type == "رواتب الموظفين":
    if emp_list:
      ref_name = st.selectbox("اختر الموظف", emp_list, key="q_emp_v44")
    else:
      st.error("⚠️ لا يوجد موظفون مسجلون.")

  description = st.text_area("البيان / التفاصيل", key="q_desc_v44")

  if st.button("حفظ السند المالي", key="q_save_btn_v44"):
  ...
  if df_raw.empty:
    ...
    for idx, row in df_raw.iterrows():
      ...
      c3.write(details)
      if c4.button("🗑️ حذف", key=f"del_v44_{row['id']}"):
        conn = get_db_connection()
        c = conn.cursor()
...
# --- 3. الصناديق ---
  ...
  headers = [
      "اسم الصندوق",
      "إجمالي المدفوع للشيخ / المقبوضات ($)",
      "إجمالي المسحوب من الشيخ / المصروفات ($)",
  ...
  for f in df_funds["name"]:
    if f == "ذمة وسلف الشيخ عبد الكريم":
      ...
      if net_bal > 0:
        ...
        text_bal = f"${abs(net_bal):,.0f} (مطلب عليك للمسجد / الصناديق)"
      else:
        text_bal = "$0 (مصفّر)"
...
# --- 4. حساب الشيخ ---
  ...
  if df_trans.empty:
    ...
    if status > 0:
      ...
      st.warning(
          f"⚖️ الميزان الحالي: أنت مدين للمسجد بمبلغ {abs(status):,.0f}$ (مطلب"
          " عليك للمسجد / الصناديق)"
      )
    else:
...
# --- 5. الرواتب ---
  ...
  col1, col2 = st.columns(2)
  emp_name = col1.text_input("اسم الموظف كاملاً", key="emp_n_v44")
  ...
  if st.button("حفظ الموظف الجديد", key="emp_save_v44"):
    if emp_name:
      conn = get_db_connection()
      c = conn.cursor()
      c.execute(
...
# --- 6. التقارير ---
  ...
  rep_type = st.selectbox(
      "نوع التقرير المراد عرضه", ["يومي", "شهري", "سنوي"], key="rep_t_v44"
  )
  conn = get_db_connection()
  ...
  if df_report.empty:
    ...
    if rep_type == "يومي":
      sel_date = st.date_input("اختر اليوم", datetime.now(), key="rep_d_v44")
      df_filtered = df_report[df_report["parsed_date"].dt.date == sel_date]
    elif rep_type == "شهري":
      sel_month = st.slider(
          "اختر الشهر", 1, 12, int(datetime.now().month), key="rep_m_v44"
      )
...
# --- 7. الإعدادات ---
  ...
  if st.button("تحديث سعر الصرف الآن", key="set_save_r_v44"):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
  ...
  uploaded_file = st.file_uploader(
      ...
      type=["db"],
      key="restore_uploader_v44",
  )
  if uploaded_file is not None:
    if st.button(
        "⚙️ اضغط هنا لتأكيد استعادة البيانات الآن",
        key="confirm_restore_btn_v44",
    ):
      try:
