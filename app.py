...
page = st.sidebar.radio("انتقل إلى:", ["🏠 الرئيسية (لوحة التحكم)", "📝 القيود اليومية", "💵 الصناديق", "👤 حساب الشيخ عبد الكريم", "👥 الرواتب", "📊 التقارير", "⚙️ الإعدادات"], key="side_nav_v37")
...
def calculate_sheikh_final_balance(df):
    ...
    for _, row in df.iterrows():
        ...
        # 2. إذا تم صرف مبلغ من صندوق المسجد العام وكان مخصصاً لحساب الشيخ أو صندوق الشيخ (الاسترداد والتسديد)
...
if page == "🏠 الرئيسية (لوحة التحكم)":
    ...
    # تحديث طريقة جمع المداخيل والمصاريف العامة: نجمع كل شيء عدا الزكاة
    ...
    if df_trans.empty or df_trans[df_trans['type'] == 'صرف'].empty:
        ...
        # استثناء عمليات الزكاة من ملخص المصاريف العامة أيضاً
...
# --- 2. القيود اليومية ---
    ...
    t_date = col1.date_input("التاريخ", datetime.now(), key="q_date_v37")
    t_type = col2.selectbox("نوع العملية", ["قبض", "صرف"], key="q_type_v37")
    
    usd_amount_raw = col1.number_input("المبلغ بالدولار ($)", min_value=0.0, step=1.0, value=None, placeholder="اكتب المبلغ بالدولار مباشرة...", key="q_usd_v37")
    lbp_amount_raw = col2.number_input("المبلغ بالليرة (ل.ل)", min_value=0.0, step=1000.0, value=None, placeholder="اكتب المبلغ بالليرة مباشرة...", key="q_lbp_v37")
    ...
    fund = col1.selectbox("الصندوق المتأثر", funds_list, key="q_fund_v37")
    account_type = col2.selectbox("نوع الحساب", ["عام", "حساب الشيخ عبد الكريم", "رواتب الموظفين"], key="q_acc_type_v37")
    
    ref_name = ""
    if account_type == "رواتب الموظفين":
        if emp_list: ref_name = st.selectbox("اختر الموظف", emp_list, key="q_emp_v37")
        else: st.error("⚠️ لا يوجد موظفون مسجلون.")
        
    description = st.text_area("البيان / التفاصيل", key="q_desc_v37")
    
    if st.button("حفظ السند المالي", key="q_save_btn_v37"):
    ...
    if df_raw.empty:
        ...
        for idx, row in df_raw.iterrows():
            ...
            c3.write(details)
            if c4.button("🗑️ حذف", key=f"del_v37_{row['id']}"):
                conn = get_db_connection()
                c = conn.cursor()
    ...
    col1, col2 = st.columns(2)
    emp_name = col1.text_input("اسم الموظف كاملاً", key="emp_n_v37")
    
    emp_salary_raw = col2.number_input("الراتب الشهري المحدد ($)", min_value=0, step=50, value=None, placeholder="مثال: 200...", key="emp_s_v37")
    emp_salary = emp_salary_raw if emp_salary_raw is not None else 0.0
    
    if st.button("حفظ الموظف الجديد", key="emp_save_v37"):
        if emp_name:
            conn = get_db_connection()
            c = conn.cursor()
    ...
    rep_type = st.selectbox("نوع التقرير المراد عرضه", ["يومي", "شهري", "سنوي"], key="rep_t_v37")
    conn = get_db_connection()
    ...
    if df_report.empty:
        ...
        if rep_type == "يومي":
            sel_date = st.date_input("اختر اليوم", datetime.now(), key="rep_d_v37")
            df_filtered = df_report[df_report['parsed_date'].dt.date == sel_date]
        elif rep_type == "شهري":
            sel_month = st.slider("اختر الشهر", 1, 12, int(datetime.now().month), key="rep_m_v37")
            df_filtered = df_report[df_report['parsed_date'].dt.month == sel_month]
        else:
            sel_year = st.number_input("حدد السنة", min_value=2020, value=int(datetime.now().year), key="rep_y_v37")
    ...
    new_rate = st.number_input("تحديث سعر صرف الدولار مقابل الليرة اللبنانية", value=dollar_rate, step=500.0, key="set_r_v37")
    if st.button("تحديث سعر الصرف الآن", key="set_save_r_v37"):
        conn = get_db_connection()
        c = conn.cursor()
    ...
    uploaded_file = st.file_uploader("📤 اختر ملف المحفوظات (Backup) من هاتفك لاستعادة الحسابات فوراً", type=["db"], key="restore_uploader_v37")
    if uploaded_file is not None:
        if st.button("⚙️ اضغط هنا لتأكيد استعادة البيانات الآن", key="confirm_restore_btn_v37"):
            try:
    ...
    confirm_reset = st.checkbox("أوافق على حذف وتصفير جميع السندات والعمليات الحسابية نهائياً من البرنامج", key="confirm_reset_v37")
    if st.button("🔴 تصفير كافة العمليات الحسابية الآن", key="reset_btn_v37"):
        if confirm_reset:
            try:
                conn = get_db_connection()
                c = conn.cursor()
