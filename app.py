...
def render_custom_html_table(headers, rows):
    ...
    for row in rows:
        html += "<tr>"
        for cell in row:
            display_cell = "" if cell is None or cell == 0 or cell == "$0" or cell == "0.0$" else cell
            html += f"<td>{display_cell}</td>"
        html += "</tr>"
    html += "</tbody></table>"
...
if page == "🏠 الرئيسية (لوحة التحكم)":
    ...
    col1.metric("💰 رصيد صندوق المسجد المدمج الحالي ($)", f"${current_balance:,.0f}" if current_balance != 0 else "-")
    col2.metric("🟢 إجمالي المقبوضات العامة (عدا الزكاة) ($)", f"${total_in:,.0f}" if total_in != 0 else "-")
    col3.metric("🔴 إجمالي المصروفات العامة (عدا الزكاة) ($)", f"${total_out:,.0f}" if total_out != 0 else "-")
    ...
    if not all_distinct_workers:
        ...
        for worker in all_distinct_workers:
            ...
            s_sal = f"${assigned_salary:,.0f}" if assigned_salary > 0 else "-"
            s_paid = f"${amount_paid_this_month:,.0f}" if amount_paid_this_month > 0 else "-"
            s_rem = f"${amount_remaining:,.0f}" if amount_remaining != 0 else "-"
            
            rows.append([display_name, s_sal, s_paid, s_rem])
    ...
    if df_trans.empty or df_trans[df_trans['type'] == 'صرف'].empty:
        ...
        if df_ops.empty:
            ...
            rows = [[row['description'], f"${row['total_usd']:,.0f}" if row['total_usd'] > 0 else "-"] for _, row in df_ops_grouped.iterrows()]
            render_custom_html_table(headers, rows)
    ...
    for f in funds_list:
        if f == "ذمة وسلف الشيخ عبد الكريم":
            ...
            net_f = f_in - f_out
            rows.append([f, f"${net_f:,.0f}" if net_f != 0 else "-"])
    render_custom_html_table(headers, rows)

# --- 2. القيود اليومية ---
elif page == "📝 القيود اليومية":
    st.title("📝 تسجيل وتعديل القيود اليومية للمسجد")
    
    funds_list = get_funds()
    df_emps = get_employees_df()
    emp_list = df_emps["name"].dropna().tolist() if not df_emps.empty else []
    df_trans = get_transactions_df()
    
    tab_add, tab_edit = st.tabs(["➕ إضافة سند جديد", "✏️ تعديل سند سابق"])
    ...
    with tab_add:
        ...
        t_type = st.selectbox("نوع العملية", ["صرف", "قبض"], key="add_t_type")

        if t_type == "صرف":
            account_options = ["عام", "رواتب الموظفين", "حساب الشيخ عبد الكريم"]
        else:
            account_options = ["عام", "حساب الشيخ عبد الكريم"]
            
        account_type = st.selectbox("نوع الحساب", account_options, key="add_acc_type")
        ...
        with st.form("daily_form_v69", clear_on_submit=True):
            ...
            usd_input_str = col1.text_input("المبلغ بالدولار ($)", value="", placeholder="اتركه فارغاً أو اكتب المبلغ...", key="add_usd_str")
            lbp_input_str = col2.text_input("المبلغ بالليرة (ل.ل)", value="", placeholder="اتركه فارغاً أو اكتب المبلغ...", key="add_lbp_str")

            usd_amount = parse_float_input(usd_input_str)
            lbp_amount = parse_float_input(lbp_input_str)
            ...
            fund = col1.selectbox("الصندوق المتأثر", funds_list, key="add_fund_sel")
            description = st.text_area("البيان والتفاصيل", key="add_desc_area")
    ...
    with tab_edit:
        st.subheader("✏️ تعديل سند مالي سابق للمسجد")
        if df_trans.empty:
            st.info("💡 لا توجد سندات مسجلة للتعديل.")
        else:
            trans_ids = df_trans['id'].tolist()
            selected_trans_id = st.selectbox("اختر رقم السند المطلوب تعديله:", trans_ids, key="edit_trans_sel_id")
            ...
            with st.form(f"edit_trans_form_{selected_trans_id}"):
                ...
                curr_t_type = row_to_edit.get('type', 'صرف')
                ed_type_idx = 0 if curr_t_type == 'صرف' else 1
                ed_type = st.selectbox("نوع العملية", ["صرف", "قبض"], index=ed_type_idx, key="ed_t_type")
                
                curr_acc = row_to_edit.get('account_type', 'عام')
                acc_options = ["عام", "رواتب الموظفين", "حساب الشيخ عبد الكريم"] if ed_type == "صرف" else ["عام", "حساب الشيخ عبد الكريم"]
                try:
                    acc_idx = acc_options.index(curr_acc)
                except:
                    acc_idx = 0
                ed_acc_type = st.selectbox("نوع الحساب", acc_options, index=acc_idx, key="ed_acc_type")
                
                ed_ref_name = row_to_edit.get('ref_name', '')
                if ed_acc_type == "رواتب الموظفين" and emp_list:
                    try:
                        ref_idx = emp_list.index(ed_ref_name)
                    except:
                        ref_idx = 0
                    ed_ref_name = st.selectbox("الموظف المستفيد:", emp_list, index=ref_idx, key="ed_emp_dropdown")
                
                col_e1, col_e2 = st.columns(2)
                ed_usd_val = float(row_to_edit.get('amount_usd', 0.0) or 0.0)
                ed_lbp_val = float(row_to_edit.get('amount_lbp', 0.0) or 0.0)
                
                ed_usd_str = col_e1.text_input("المبلغ بالدولار ($)", value=str(ed_usd_val) if ed_usd_val > 0 else "")
                ed_lbp_str = col_e2.text_input("المبلغ بالليرة (ل.ل)", value=str(ed_lbp_val) if ed_lbp_val > 0 else "")
                
                f_parsed_usd = parse_float_input(ed_usd_str)
                f_parsed_lbp = parse_float_input(ed_lbp_str)
                f_conv = round(f_parsed_lbp / dollar_rate) if dollar_rate > 0 else 0
                f_tot = round(f_parsed_usd + f_conv)
                
                curr_fund = row_to_edit.get('fund', 'المسجد العامة')
                try:
                    f_idx = funds_list.index(curr_fund)
                except:
                    f_idx = 0
                ed_fund = col_e1.selectbox("الصندوق المتأثر", funds_list, index=f_idx, key="ed_fund_sel")
                
                ed_desc = st.text_area("البيان والتفاصيل", value=str(row_to_edit.get('description', '')))
                
                ed_submitted = st.form_submit_button("حفظ التعديلات على السند")
                ...
                if ed_submitted:
                    if f_tot == 0:
                        ...
                        upd_payload = {
                            ...
                            "description": ed_desc,
                            "type": ed_type,
                            "amount_usd": f_parsed_usd,
                            "amount_lbp": f_parsed_lbp,
                            "total_usd": f_tot,
                            "fund": ed_fund,
                            "account_type": ed_acc_type,
                            "ref_name": ed_ref_name if ed_acc_type == "رواتب الموظفين" else ""
                        }
                        supabase.table("transactions").update(upd_payload).eq("id", selected_trans_id).execute()
                        st.success(f"تم تحديث السند رقم {selected_trans_id} بنجاح!")
                        safe_rerun()
...
# --- 3. الصناديق وصندوق الجمعة ---
elif page == "💵 الصناديق وصندوق الجمعة":
    st.title("💵 إدارة وتفاصيل أرصدة الصناديق ومداخيل الجمعة")
    df_trans = get_transactions_df()
    funds_list = get_funds()

    st.markdown("### 🕌 استعلام مخصص لمداخيل (صندوق الجمعة)")
    st.markdown("أعرف كم دخل صندوق الجمعة خلال شهر، سنة، أو فترة زمنية محددة بضغطة زر:")
    
    if not df_trans.empty:
        df_trans['parsed_date'] = pd.to_datetime(df_trans['date'], errors='coerce')
        df_jumaa = df_trans[(df_trans['fund'] == 'صندوق الجمعة') & (df_trans['type'] == 'قبض')]
        
        col_j1, col_j2, col_j3 = st.columns(3)
        sel_jumaa_year = col_j1.number_input("سنة الاستعلام:", min_value=2020, value=datetime.now().year, key="j_year_sel")
        sel_jumaa_month = col_j2.selectbox("شهر الاستعلام:", ["كل السنة"] + list(range(1, 13)), index=datetime.now().month, key="j_month_sel")
        
        if sel_jumaa_month == "كل السنة":
            df_filtered_jumaa = df_jumaa[df_jumaa['parsed_date'].dt.year == sel_jumaa_year]
            period_label = f"سنة {sel_jumaa_year}"
        else:
            df_filtered_jumaa = df_jumaa[(df_jumaa['parsed_date'].dt.year == sel_jumaa_year) & (df_jumaa['parsed_date'].dt.month == int(sel_jumaa_month))]
            period_label = f"شهر {sel_jumaa_month} لسنة {sel_jumaa_year}"
            
        total_jumaa_income = df_filtered_jumaa['total_usd'].sum() if not df_filtered_jumaa.empty else 0.0
        col_j3.metric(f"💵 إجمالي مداخيل صندوق الجمعة لـ ({period_label})", f"${total_jumaa_income:,.0f}" if total_jumaa_income > 0 else "$0")
        
        if not df_filtered_jumaa.empty:
            with st.expander("📋 عرض تفاصيل سندات صندوق الجمعة لهذه الفترة"):
                j_headers = ["رقم السند", "التاريخ", "البيان والتفاصيل", "المبلغ الإجمالي ($)"]
                j_rows = [[r['id'], r['date'], r['description'], f"${float(r['total_usd']):,.0f}"] for _, r in df_filtered_jumaa.iterrows()]
                render_custom_html_table(j_headers, j_rows)
    else:
        st.info("💡 لا توجد بيانات مسجلة بعد.")

    st.write("---")
    st.markdown("### 📊 الملخص العام لكافة الصناديق")
    ...
    for f in funds_list:
        if f == "ذمة وسلف الشيخ عبد الكريم":
            ...
            if net_bal > 0:
                ...
                text_bal = "-"
            rows.append([f, f"${sh_paid:,.0f}" if sh_paid > 0 else "-", f"${sh_rec:,.0f}" if sh_rec > 0 else "-", text_bal])
        else:
            f_in = df_trans[(df_trans['fund'] == f) & (df_trans['type'] == 'قبض')]['total_usd'].sum() if not df_trans.empty else 0.0
            f_out = df_trans[(df_trans['fund'] == f) & (df_trans['type'] == 'صرف')]['total_usd'].sum() if not df_trans.empty else 0.0
            net_f = f_in - f_out
            rows.append([f, f"${f_in:,.0f}" if f_in > 0 else "-", f"${f_out:,.0f}" if f_out > 0 else "-", f"${net_f:,.0f}" if net_f != 0 else "-"])
    render_custom_html_table(headers, rows)
...
# --- 5. الرواتب ---
    ...
    if st.button("حفظ الموظف الجديد", key="emp_save_v69"):
        if emp_name:
            ...
            st.success(f"تم حفظ الموظف {clean_name} بنجاح!")
            safe_rerun()
...
# --- 6. التقارير ---
    ...
    if df_report.empty:
        ...
        if df_filtered.empty:
            ...
            mc1.metric("🟢 إجمالي المقبوضات (عدا الزكاة)", f"${total_in_rep:,.0f}" if total_in_rep > 0 else "-")
            mc2.metric("🔴 إجمالي المصروفات (عدا الزكاة)", f"${total_out_rep:,.0f}" if total_out_rep > 0 else "-")
            mc3.metric("💰 الصافي للفترة", f"${net_rep:,.0f}" if net_rep != 0 else "-")
...
# --- 7. حسابي الشخصي ---
    ...
    pc1.metric("💰 صافي رصيدك الشخصي الحالي ($)", f"${p_balance:,.0f}" if p_balance != 0 else "-")
    pc2.metric("🟢 إجمالي المداخيل الشخصية ($)", f"${p_in:,.0f}" if p_in > 0 else "-")
    pc3.metric("🔴 إجمالي المصاريف الشخصية ($)", f"${p_out:,.0f}" if p_out > 0 else "-")

    st.write("---")
    
    p_tab_add, p_tab_edit, p_tab_monthly = st.tabs(["➕ إضافة حركة شخصية", "✏️ تعديل حركة شخصية", "📅 تقرير الحساب الشخصي لكل شهر"])
    ...
    with p_tab_add:
        ...
        with st.form("personal_form_v69", clear_on_submit=True):
            ...
            p_usd_str = col1.text_input("المبلغ بالدولار ($)", value="", placeholder="اتركه فارغاً أو اكتب المبلغ...", key="p_add_usd")
            p_lbp_str = col2.text_input("المبلغ بالليرة (ل.ل)", value="", placeholder="اتركه فارغاً أو اكتب المبلغ...", key="p_add_lbp")

            p_usd = parse_float_input(p_usd_str)
            p_lbp = parse_float_input(p_lbp_str)
            ...
            p_category = st.text_input("التصنيف (مثال: راتب شخصي، أجار منزل، طعام...)", key="p_add_cat")
            p_desc = st.text_area("البيان والتفاصيل الشخصية", key="p_add_desc")
    ...
    with p_tab_edit:
        st.subheader("✏️ تعديل حركة شخصية سابقة")
        if df_personal.empty:
            st.info("💡 لا توجد حركات شخصية للتعديل.")
        else:
            pers_ids = df_personal['id'].tolist()
            sel_pers_edit_id = st.selectbox("اختر رقم السند الشخصي المطلوب تعديله:", pers_ids, key="sel_edit_pers_id")
            
            p_row_to_edit = df_personal[df_personal['id'] == sel_pers_edit_id].iloc[0]
            
            with st.form(f"edit_pers_form_{sel_pers_edit_id}"):
                try:
                    pe_date_val = datetime.strptime(str(p_row_to_edit['date'])[:10], "%Y-%m-%d").date()
                except:
                    pe_date_val = datetime.now().date()
                
                pe_date = st.date_input("التاريخ", pe_date_val)
                
                curr_p_type = p_row_to_edit.get('type', 'صرف (مصروف)')
                pe_type_idx = 0 if curr_p_type == 'قبض (مدخول)' else 1
                pe_type = st.selectbox("نوع الحركة", ["قبض (مدخول)", "صرف (مصروف)"], index=pe_type_idx, key="pe_type_sel")
                
                pe_col1, pe_col2 = st.columns(2)
                pe_usd_val = float(p_row_to_edit.get('amount_usd', 0.0) or 0.0)
                pe_lbp_val = float(p_row_to_edit.get('amount_lbp', 0.0) or 0.0)
                
                pe_usd_str = pe_col1.text_input("المبلغ بالدولار ($)", value=str(pe_usd_val) if pe_usd_val > 0 else "", key="pe_usd_input")
                pe_lbp_str = pe_col2.text_input("المبلغ بالليرة (ل.ل)", value=str(pe_lbp_val) if pe_lbp_val > 0 else "", key="pe_lbp_input")
                
                pe_f_usd = parse_float_input(pe_usd_str)
                pe_f_lbp = parse_float_input(pe_lbp_str)
                pe_conv = round(pe_f_lbp / dollar_rate) if dollar_rate > 0 else 0
                pe_tot = round(pe_f_usd + pe_conv)
                
                pe_cat = st.text_input("التصنيف", value=str(p_row_to_edit.get('category', 'عام')), key="pe_cat_input")
                pe_desc = st.text_area("البيان والتفاصيل الشخصية", value=str(p_row_to_edit.get('description', '')), key="pe_desc_input")
                
                pe_submitted = st.form_submit_button("حفظ التعديلات على الحركة الشخصية")
                ...
                if pe_submitted:
                    if pe_tot == 0:
                        ...
                        pe_payload = {
                            ...
                            "amount_lbp": pe_f_lbp,
                            "total_usd": pe_tot,
                            "category": pe_cat if pe_cat else "عام"
                        }
                        supabase.table("personal_transactions").update(pe_payload).eq("id", sel_pers_edit_id).execute()
                        st.success(f"تم تحديث الحركة الشخصية رقم {sel_pers_edit_id} بنجاح!")
                        safe_rerun()

    with p_tab_monthly:
        st.subheader("📅 تقرير الحساب الشخصي مفصلاً لكل شهر")
        if df_personal.empty:
            st.info("💡 لا توجد حركات شخصية مسجلة بعد.")
        else:
            df_personal['parsed_date'] = pd.to_datetime(df_personal['date'], errors='coerce')
            df_personal['year_month'] = df_personal['parsed_date'].dt.to_period('M')
            
            unique_months = sorted(df_personal['year_month'].dropna().unique(), reverse=True)
            
            if not unique_months:
                st.info("💡 لا توجد تواريخ صالحة لفرز الشهور.")
            else:
                month_labels = [str(m) for m in unique_months]
                selected_month_str = st.selectbox("اختر الشهر المطلوب لعرض تقريره الشخصي:", month_labels, key="sel_pers_month_rep")
                
                sel_period = pd.Period(selected_month_str, freq='M')
                df_month_pers = df_personal[df_personal['year_month'] == sel_period]
                
                m_in = df_month_pers[df_month_pers['type'] == 'قبض (مدخول)']['total_usd'].sum()
                m_out = df_month_pers[df_month_pers['type'] == 'صرف (مصروف)']['total_usd'].sum()
                m_net = m_in - m_out
                
                st.markdown(f"#### ملخص شهر: {selected_month_str}")
                mc_1, mc_2, mc_3 = st.columns(3)
                mc_1.metric("🟢 إجمالي المداخيل للشهر", f"${m_in:,.0f}" if m_in > 0 else "-")
                mc_2.metric("🔴 إجمالي المصاريف للشهر", f"${m_out:,.0f}" if m_out > 0 else "-")
                mc_3.metric("💰 صافي الشهر", f"${m_net:,.0f}" if m_net != 0 else "-")
                
                st.write("---")
                m_headers = ["رقم السند", "التاريخ", "نوع الحركة", "التصنيف", "البيان والتفاصيل", "المبلغ الإجمالي ($)"]
                m_rows = []
                for _, r in df_month_pers.iterrows():
                ...
                with pd.ExcelWriter(pm_output, engine='openpyxl') as writer:
                    df_month_export.to_excel(writer, index=False, sheet_name=f'Personal_{selected_month_str}')
                    
                    pm_book = writer.book
                    pm_sheet = writer.sheets[f'Personal_{selected_month_str}']
                    pm_sheet.sheet_view.rightToLeft = True
                    
                    pm_fill = PatternFill(start_color="004D40", end_color="004D40", fill_type="solid")
                    pm_font = Font(color="D4AF37", bold=True)
                    
                    for col_num, value in enumerate(df_month_export.columns.values, 1):
                        cell = pm_sheet.cell(row=1, column=col_num)
                        cell.fill = pm_fill
                        cell.font = pm_font
                        cell.alignment = Alignment(horizontal="center", vertical="center")
                        pm_sheet.column_dimensions[get_column_letter(col_num)].width = 20

                    for row_num in range(2, pm_sheet.max_row + 1):
                        for col_num in range(1, pm_sheet.max_column + 1):
                            cell = pm_sheet.cell(row=row_num, column=col_num)
                            cell.alignment = Alignment(horizontal="right")

                pm_excel_data = pm_output.getvalue()
                st.download_button(
                    label=f"📥 تحميل تقرير شهر ({selected_month_str}) الشخصي (xlsx)",
                    data=pm_excel_data,
                    file_name=f"personal_report_{selected_month_str}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key=f"export_pers_month_{selected_month_str}"
                )

    st.write("---")
    st.subheader("📋 سجل الحركات الشخصية السابقة وتصدير التقارير العامة")
    ...
    if df_personal.empty:
        ...
        for _, row in df_personal.iterrows():
            r_id = row.get('id', '')
            r_date = str(row.get('date', ''))[:10]
            r_type = row.get('type', '')
            r_cat = row.get('category', 'عام')
            r_desc = row.get('description', '')
            r_tot = float(row.get('total_usd', 0.0))
            rows.append([f"#سند {r_id}", r_date, r_type, r_cat, r_desc, f"${r_tot:,.0f}" if r_tot > 0 else "-"])
        
        render_custom_html_table(headers, rows)

        p_output = io.BytesIO()
        df_personal_clean = df_personal.drop(columns=[c for c in ['parsed_date', 'year_month'] if c in df_personal.columns])
        with pd.ExcelWriter(p_output, engine='openpyxl') as writer:
            df_personal_clean.to_excel(writer, index=False, sheet_name='PersonalReport')
            
            p_workbook = writer.book
            p_worksheet = writer.sheets['PersonalReport']
            p_worksheet.sheet_view.rightToLeft = True
            
            p_header_fill = PatternFill(start_color="004D40", end_color="004D40", fill_type="solid")
            p_header_font = Font(color="D4AF37", bold=True)
            
            for col_num, value in enumerate(df_personal_clean.columns.values, 1):
                cell = p_worksheet.cell(row=1, column=col_num)
                cell.fill = p_header_fill
                cell.font = p_header_font
                cell.alignment = Alignment(horizontal="center", vertical="center")
                p_worksheet.column_dimensions[get_column_letter(col_num)].width = 20

            for row_num in range(2, p_worksheet.max_row + 1):
                for col_num in range(1, p_worksheet.max_column + 1):
                    cell = p_worksheet.cell(row=row_num, column=col_num)
                    cell.alignment = Alignment(horizontal="right")

        p_excel_data = p_output.getvalue()

        st.download_button(
            label="📥 تحميل تقرير الحساب الشخصي العام الشامل (xlsx)",
            data=p_excel_data,
...
# --- 8. الإعدادات ---
    ...
    if st.button("تحديث سعر الصرف الآن", key="set_save_r_v69"):
        if new_rate > 0:
            ...
            st.error("الرجاء إدخال سعر صرف صحيح.")
