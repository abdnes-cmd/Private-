...
if page == "🏠 الرئيسية (لوحة التحكم)":
  st.markdown(
      "<h1 style='text-align: center;'>لوحة التحكم المالية</h1>",
      unsafe_allow_html=True,
  )
  st.markdown(
  ...
  # جلب البيانات أولاً لتفادي NameError
  conn = get_db_connection()
  ...
  # حساب المبالغ بالليرة والدولار معاً
  if not df_trans.empty:
    total_in_usd = df_trans[
        (df_trans["fund"] != "الزكاة") & (df_trans["type"] == "قبض")
    ]["total_usd"].sum()
    total_in_lbp = df_trans[
        (df_trans["fund"] != "الزكاة") & (df_trans["type"] == "قبض")
    ]["amount_lbp"].sum()

    total_out_usd = df_trans[
        (df_trans["fund"] != "الزكاة") & (df_trans["type"] == "صرف")
    ]["total_usd"].sum()
    total_out_lbp = df_trans[
        (df_trans["fund"] != "الزكاة") & (df_trans["type"] == "صرف")
    ]["amount_lbp"].sum()
  else:
    total_in_usd, total_in_lbp = 0.0, 0.0
    total_out_usd, total_out_lbp = 0.0, 0.0
...
# --- 2. القيود اليومية ---
  ...
  t_date = col1.date_input("التاريخ", datetime.now(), key="q_date_v39")
  t_type = col2.selectbox("نوع العملية", ["قبض", "صرف"], key="q_type_v39")
  ...
  fund = col1.selectbox("الصندوق المتأثر", funds_list, key="q_fund_v39")
  ...
  if account_type == "رواتب الموظفين":
    if emp_list:
      ref_name = st.selectbox("اختر الموظف", emp_list, key="q_emp_v39")
    else:
      st.error("⚠️ لا يوجد موظفون مسجلون.")

  description = st.text_area("البيان / التفاصيل", key="q_desc_v39")

  if st.button("حفظ السند المالي", key="q_save_btn_v39"):
    if total_calculated_usd == 0 and lbp_amount == 0:
  ...
  if df_raw.empty:
    ...
    for idx, row in df_raw.iterrows():
      ...
      c2.write(f"**📅:** {row['date']}")
      details = (
          f"【 {row['type']} 】 بمبلغ **{row['total_usd']:,.0f}$** ("
          f" {row['amount_lbp']:,.0f} ل.ل ) | {row['description']}"
      )
      if row["ref_name"]:
        details += f" ({row['ref_name']})"
      c3.write(details)
      if c4.button("🗑️ حذف", key=f"del_v39_{row['id']}"):
        conn = get_db_connection()
        c = conn.cursor()
  ...
  if df_trans.empty:
    ...
    if status > 0:
      st.success(f"⚖️ الميزان الحالي: المسجد مدين لك بمبلغ {status:,.0f}$")
    elif status < 0:
      st.warning(f"⚖️ الميزان الحالي: أنت مدين للمسجد بمبلغ {abs(status):,.0f}$")
    else:
      st.info("⚖️ الميزان الحالي: الحساب متقاص تماماً ($0)!")
  ...
  col1, col2 = st.columns(2)
  emp_name = col1.text_input("اسم الموظف كاملاً", key="emp_n_v39")
  ...
  if st.button("حفظ الموظف الجديد", key="emp_save_v39"):
    if emp_name:
      conn = get_db_connection()
      c = conn.cursor()
      c.execute(
  ...
  if df_report.empty:
    ...
    if df_filtered.empty:
      ...
      for _, r in df_filtered.iterrows():
        ...
        lbp_str = f"{r['amount_lbp']:,.0f} ل.ل" if r["amount_lbp"] > 0 else "-"
        rows.append([
            r["id"],
            r["date"],
            r["type"],
      ...
      # تصدير التقرير المفلتر إلى Excel
      output = io.BytesIO()
      with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        df_export = df_filtered.drop(columns=["parsed_date"])
        df_export.to_excel(writer, index=False, sheet_name="التقرير المالي")
      excel_data = output.getvalue()

      st.download_button(
          label="📥 تحميل التقرير المعروض بصيغة Excel",
          data=excel_data,
          file_name=f"mosque_report_{rep_type}_{datetime.now().strftime('%Y%m%d')}.xlsx",
          mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          key="export_excel_v39",
      )
  ...
  if st.button("تحديث سعر الصرف الآن", key="set_save_r_v39"):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
  ...
  uploaded_file = st.file_uploader(
      ...
      type=["db"],
      key="restore_uploader_v39",
  )
  if uploaded_file is not None:
    if st.button(
        "⚙️ اضغط هنا لتأكيد استعادة البيانات الآن",
        key="confirm_restore_btn_v39",
    ):
      try:
  ...
  confirm_reset = st.checkbox(
      ...
      key="confirm_reset_v39",
  )
  if st.button("🔴 تصفير كافة العمليات الحسابية الآن", key="reset_btn_v39"):
    if confirm_reset:
      try:
        conn = get_db_connection()
        c = conn.cursor()
