...
# --- 2. القيود اليومية ---
  ...
  t_date = col1.date_input("التاريخ", datetime.now(), key="q_date_v40")
  t_type = col2.selectbox("نوع العملية", ["قبض", "صرف"], key="q_type_v40")
  ...
  fund = col1.selectbox("الصندوق المتأثر", funds_list, key="q_fund_v40")
  ...
  if account_type == "رواتب الموظفين":
    if emp_list:
      ref_name = st.selectbox("اختر الموظف", emp_list, key="q_emp_v40")
    else:
      st.error("⚠️ لا يوجد موظفون مسجلون.")

  description = st.text_area("البيان / التفاصيل", key="q_desc_v40")

  if st.button("حفظ السند المالي", key="q_save_btn_v40"):
  ...
  if df_raw.empty:
    ...
    for idx, row in df_raw.iterrows():
      ...
      c2.write(f"**📅:** {row['date']}")
      details = (
          f"【 {row['type']} 】 | كاش: **${row['amount_usd']:,.0f}** | ليرة:"
          f" **{row['amount_lbp']:,.0f} ل.ل** | الإجمالي:"
          f" **${row['total_usd']:,.0f}** | {row['description']}"
      )
      if row["ref_name"]:
        details += f" ({row['ref_name']})"
      c3.write(details)
      if c4.button("🗑️ حذف", key=f"del_v40_{row['id']}"):
        conn = get_db_connection()
        c = conn.cursor()
...
# --- 5. الرواتب ---
  ...
  col1, col2 = st.columns(2)
  emp_name = col1.text_input("اسم الموظف كاملاً", key="emp_n_v40")
  ...
  if st.button("حفظ الموظف الجديد", key="emp_save_v40"):
    if emp_name:
      conn = get_db_connection()
      c = conn.cursor()
      c.execute(
...
# --- 6. التقارير (مع فصل الكاش عن الليرة) ---
elif page == "📊 التقارير":
  st.title("📊 التقارير المالية والطباعة")
  rep_type = st.selectbox(
      "نوع التقرير المراد عرضه", ["يومي", "شهري", "سنوي"], key="rep_t_v40"
  )
  conn = get_db_connection()
  ...
  if df_report.empty:
    ...
    if rep_type == "يومي":
      sel_date = st.date_input("اختر اليوم", datetime.now(), key="rep_d_v40")
      df_filtered = df_report[df_report["parsed_date"].dt.date == sel_date]
    elif rep_type == "شهري":
      sel_month = st.slider(
          "اختر الشهر", 1, 12, int(datetime.now().month), key="rep_m_v40"
      )
...
# --- 7. الإعدادات ---
  ...
  if st.button("تحديث سعر الصرف الآن", key="set_save_r_v40"):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
  ...
  uploaded_file = st.file_uploader(
      ...
      type=["db"],
      key="restore_uploader_v40",
  )
  if uploaded_file is not None:
    if st.button(
        "⚙️ اضغط هنا لتأكيد استعادة البيانات الآن",
        key="confirm_restore_btn_v40",
    ):
      try:
